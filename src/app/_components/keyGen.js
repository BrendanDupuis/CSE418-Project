...
export async function genUserKeys(userName, password){
    ...
    const derivedBits = await crypto.subtle.deriveBits( { name: "PBKDF2",salt: encoder.encode(salt),iterations: 100000,
      hash: "SHA-256"},encodePassKey,256);
    ...
    const privateKey = await crypto.subtle.importKey("raw",derivedBits,{ name: "ECDH", namedCurve: "P-256" },
